#!/usr/bin/env python3
"""
the platform — Campaign Proposal PDF Builder.

Reads a markdown source and renders a branded PDF with:
- cover page (logo + title + client + date)
- centered watermark logo on every content page (low opacity)
- LF colors (#4A7FD4 H2, #1E2A4A H1, body black)
- Helvetica, light/clean layout
- footer "the platform — Confidentiel" on every page
...
LF_BLUE = HexColor("#4A7FD4")
LF_DARK = HexColor("#1E2A4A")
...
def make_faded_logo(src: Path, opacity: float = 0.06) -> Path:
    """Bake a low-opacity copy of the logo for use as a watermark."""
    img = Image.open(src).convert("RGBA")
    r, g, b, a = img.split()
    a = a.point(lambda v: int(v * opacity))
    img.putalpha(a)
    out = Path("/tmp") / f"lf_watermark_{src.stem}.png"
    img.save(out)
    return out


class ProposalPDF:
    def __init__(self, output: Path, client: str, date: str, logo: Path | None):
    ...
    def draw_cover(self):
        ...
        foot = "the platform — Acquisition Meta Ads B2B"
            ...
            for j in range(n_cols):
                ...
                for line in lines:
                    ...
                    for tok, t in line:
                        ...
                        self.c.setFillColor(LF_DARK if font == FONT_B else TEXT)
...
def render(md_path: Path, out_path: Path, client: str, date: str, logo: Path | None):
    md = md_path.read_text(encoding="utf-8")
    blocks = parse_markdown(md)

    pdf = ProposalPDF(out_path, client, date, logo)
    pdf.draw_cover()
...
def main():
    ...
    render(args.input, args.output, args.client, args.date, args.logo)
